...
public class FractionRightVersion  implements Comparable<FractionRightVersion>{
	
	private long denominator;
	private long numerator;
	
	public FractionRightVersion(long numerator,long denominator) throws ArithmeticException{
    ...
    public boolean equals(FractionRightVersion f) {  
    ...
    // 利用对应小数值的大小来判断分数的大小  
    public boolean isGreater(FractionRightVersion f) {  
    ...
    private void reduct() { 
        ...
        /*
        minValue = Math.min(Math.abs(numerator), Math.abs(denominator)); 
        long nextStepFromNum = primeNumberQueue.get(primeNumberQueue.size()-1);
    	if(minValue<nextStepFromNum){
    		return;
    	}
        primeNumberQueue = PrimeNumbers.getPrimeNumberListExceptOne(minValue+1);  
        for (long i :primeNumberQueue) {
        	if(i>=nextStepFromNum){
        		continue;
        	}
        	minValue = Math.min(Math.abs(numerator), Math.abs(denominator)); 
    ...
    // 返回分数相加的结果，并且约分  
    public FractionRightVersion add(FractionRightVersion f) {  
...
//      System.out.println(fractionNumerator+":"+fractionDenominator);
        return new FractionRightVersion(fractionNumerator,fractionDenominator);  
    }  
  
    // 返回分数相减的结果，并且约分  
    public FractionRightVersion minus(FractionRightVersion f) {  
        long fractionNumerator = numerator * f.denominator - f.numerator  
                * denominator;  
        long fractionDenominator = denominator * f.denominator;  
        return new FractionRightVersion(fractionNumerator,fractionDenominator);   
    ...
    // 返回分数相乘的结果，并且约分  
    public FractionRightVersion multiply(FractionRightVersion f) {  
    ...
    // 返回分数相除的结果，并且约分  
    public FractionRightVersion divide(FractionRightVersion f) {  
    ...
    public int compareTo(FractionRightVersion o) {  
        FractionRightVersion f = (FractionRightVersion) o;  
    ...
    public boolean equals(Object obj) {  
        FractionRightVersion f = (FractionRightVersion) obj;  
